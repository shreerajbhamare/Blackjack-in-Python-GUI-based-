...
class Play:
    ...
    def Check(self):
        ...
        if(self.hDealer.softValue() > self.hPlayer.getValue() or ( self.hDealer.getValue() < 21  and self.hDealer.getValue() > self.hPlayer.getValue()) or self.hDealer.getValue()==21):
            return 0
...
def main():
        ...
        def startGame() :
            ...
            def stand():
                ...
                if(start.hDealer.getValue() == start.hPlayer.getValue() or start.hDealer.softValue() == start.hPlayer.getValue() or start.hDealer.getValue() == start.hPlayer.softValue() or start.hDealer.softValue() == start.hPlayer.softValue()):
                    Tie()
                    print("tie")
            ...
            #Hit button
            hit = Button(actions,text="HIT",command=callOnHit,activeforeground="white",activebackground="black",bg="grey",fg="white",width="8" ,relief="raised") ##d203fc
            hit['font'] = myFont
            hit.grid(sticky="W",row=0,column=0)


            #Stand Button
            stan= Button(actions,text="STAND", command=stand,activeforeground="white",activebackground="black",bg="grey",fg="white",width="8" ,relief="raised")
            stan['font'] = myFont
